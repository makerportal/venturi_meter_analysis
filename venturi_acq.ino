...
void setup () {
  analogReference(EXTERNAL); // 3.3V instead of 5V for analog values 
  pinMode(fan_pin ,OUTPUT); // setup PWM for fan

  digitalWrite(fan_pin,0); // make sure fan is off delay(2000);
  Serial.begin(9600); // start serial port delay(100);
  Serial.println("Acquisition Start"); // data acquisition start phrase 
  Serial.println("PWM [0-255],dP[Pa],Pressure [Pa],Temp. [C]"); //header 
  ...
  unsigned status;
  status = bme.begin(); // make sure BME280 was wired/is working properly if (!status) {
...
void loop() {
  ...
  // make sure if the ADC reads outside of 512+-zero_span (this increases
  // stability around 0 pressure)
  ...
  Serial.print(delta_P); // print delta P 
  Serial.print(","); // comma separate
  ...
  if (iter_change >20) { 
    pwm_val+=10; // this if (pwm_val >=pwm_max
    pwm_val = pwm_min;
    max_val = false; 
  }
  iter_change +=1; // increase iter every loop 
  }
  can be changed to desired increase in PWM
  and max_val){
  // if we’re at max, go to pwm_min
  if (pwm_val>pwm_max and pwm_val!=pwm_max){
    pwm_val = pwm_max; // if we overshoot, go to pwm_max max_val = true;
  }
